      ...
      <div className="relative z-10 animate-in fade-in duration-1000 delay-300">
        <div className="bg-brand-700/50 p-6 rounded-3xl border border-white/10 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 relative z-10">
            ...
            <div><p className="text-xs font-black">Martin K.</p><p className="text-[10px] text-brand-200">Adoptér</p></div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="relative z-10 animate-in fade-in duration-1000 delay-300">
        <div className="bg-indigo-700/50 p-6 rounded-3xl border border-white/10 relative overflow-hidden group hover:bg-indigo-700/70 transition-all">
          ...
          <div className="flex items-center gap-3 relative z-10">
            ...
            <div><p className="text-xs font-black">Jana S.</p><p className="text-[10px] text-indigo-200">Útulok Labka</p></div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className={`bg-white rounded-[2.5rem] shadow-2xl w-full transition-all duration-500 overflow-hidden border border-gray-100 ${showBenefits ? 'max-w-5xl' : 'max-w-lg'}`}>
        <div className="flex flex-col lg:flex-row min-h-[700px]">
          ...
          <div className={`${showBenefits ? 'lg:w-1/2' : 'w-full'} order-1 lg:order-2 bg-white flex flex-col`}>
            <div className={`p-8 pb-12 text-center transition-colors duration-500 ${mode === 'login' || (mode === 'register' && role === 'user') ? 'bg-brand-600' : (mode === 'register' && role === 'shelter' ? 'bg-indigo-600' : 'bg-gray-800')
              } text-white relative`}>
              <div className="flex flex-col items-center">
                ...
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-black/10 border border-white/10 text-[10px] font-black mb-4">
              ...
              {success && (
                <div className="mb-6 p-6 bg-green-50 border border-green-100 text-green-700 text-sm rounded-2xl text-center font-bold flex flex-col items-center gap-3 animate-in fade-in zoom-in-95">
                  ...
                  {(mode === 'forgot' || mode === 'update-password') && (
                    <button onClick={() => { setMode('login'); navigate('/auth', { replace: true }); }} className="mt-2 text-xs text-brand-600 underline font-black">Späť na prihlásenie</button>
                  )}
                </div>
              )}
              ...
              {!success && (
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {mode === 'register' && role === 'shelter' && (
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 mb-2 ml-1">Lokalita / Mesto</label>
                      <div className="relative group">
                  ...
                  {mode !== 'update-password' && (
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 mb-2 ml-1">E-mailová adresa</label>
                      <div className="relative group">
                  ...
                  {mode !== 'forgot' && (
                    <div className="space-y-4">
                      ...
                      {(mode === 'register' || mode === 'update-password') && (
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 mb-2 ml-1">Zopakovať heslo</label>
                          <div className="relative group">
                            <div className={`absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 transition-colors ${role === 'shelter' && mode === 'register' ? 'group-focus-within:text-indigo-500' : 'group-focus-within:text-brand-500'}`}><Lock size={20} /></div>
                  ...
                  <div className="pt-4">
                    <button type="submit" disabled={loading} className={`w-full py-5 text-white font-black text-xs rounded-2xl shadow-xl transition transform hover:-translate-y-1 disabled:opacity-70 flex items-center justify-center gap-3 ${mode === 'login' || (mode === 'register' && role === 'user') ? 'bg-brand-600 hover:bg-brand-700 shadow-brand-200' : (mode === 'register' && role === 'shelter' ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200' : 'bg-gray-800')
                      }`}>
              ...
              {mode !== 'update-password' && !success && (
                <div className="mt-10 text-center">
                  <p className="text-gray-400 text-sm font-bold">
                    ...
                    <button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); setSuccess(''); }} className={`ml-2 font-black text-xs underline underline-offset-4 ${role === 'shelter' && mode === 'register' ? 'text-indigo-600' : 'text-brand-600'}`}>
